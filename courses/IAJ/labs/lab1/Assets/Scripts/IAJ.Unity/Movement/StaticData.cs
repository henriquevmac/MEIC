...
namespace Assets.Scripts.IAJ.Unity.Movement
{
    public class StaticData
    {
        ...
        public virtual void Integrate(MovementOutput movement, float duration)
        {
            ...
            this.Orientation %= MathConstants.MATH_2PI;
        ...
        public bool ApplyWorldLimit(float xWorldSize, float zWorldSize)
        {
            ...
            bool ret = false;
            if (position.x < -xWorldSize-6)
            {
                ret = true;
                position.x = xWorldSize;
            }
            else if (position.x > xWorldSize+6)
            {
                ret = true;
                position.x = -xWorldSize;
            }
            if (position.z < -zWorldSize-4)
            {
                ret = true;
                position.z = zWorldSize;
            }
            else if (position.z > zWorldSize+4)
            {
                ret = true;
                position.z = -zWorldSize;
            }
        ...
        /**
         * Returns a unit vector in the direction of the currentg
         * orientation.
         */
