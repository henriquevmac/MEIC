...
public class PathfindingManager : MonoBehaviour
{
    ...
    public uint nodesPerFrame = 0;
    public bool tieBreaking = false;
    ...
    [Header("Gateway A* Settings")]
    public bool preprocessOnStart = false;
    public KeyCode preprocessKey = KeyCode.P;
    public KeyCode visualizeGatewayClustersKey = KeyCode.V;
    public GatewayClusterMode gatewayClusterMode = GatewayClusterMode.PaperAutomatic;
    public int gatewayRegionSize = 10;
    ...
    private bool showingGatewayClusters;
    ...
    private void Start()
    {
        ...
        float tieBreakingWeight = tieBreaking ? 0.200f : 0f;
        ...
        switch (this.aStarType)
        {
            ...
            case AStarType.GatewayAstar:
                throw new NotImplementedException("Gateway A* is not implemented in this version of the code.");
            default:
                throw new Exception();
        }      

        this.pathfinding.NodesPerFrame = this.nodesPerFrame;

        visualGrid.GridMapVisual(width, height, cellSize, textLines, gridGraph.grid);

        if (this.preprocessOnStart)
            PreprocessPathfinding();
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (node != null && node.isWalkable)
            {
                ...
                else
                {
                    ...
                    this.showingGatewayClusters = false;
                    startingX = positionX;
                    startingY = positionY;
                    this.visualGrid.SetObjectColor(startingX, startingY, Color.cyan);
                }
    ...
    void InputHandler()
    {
        ...
        if (Input.GetKeyDown(preprocessKey))
            PreprocessPathfinding();
    ...
    public void PreprocessPathfinding()
    {
        //ToDo: Implement preprocessing logic for pathfinding if needed.
    }  
