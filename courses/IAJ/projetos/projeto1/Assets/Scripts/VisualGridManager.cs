...
public class VisualGridManager : MonoBehaviour
{
    ...
    public void ClearGrid()
    {
        for (int x = 0; x < this.width; x++)
            for (int y = 0; y <this.height; y++)
            {
                ...
                this.SetObjectText(x, y, showCoordinates ? node.ToString() : string.Empty, Color.black, 40);
    ...
    public void SetObjectText(int x, int y, string text, Color color, int fontSize)
    {
        TextMesh textMesh = visualGrid[x, y].GetComponentInChildren<TextMesh>();
